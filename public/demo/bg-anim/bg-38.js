const shader = {
  ...
  float world_sdf(in vec3 p) {
    ...
    p.xy *= rot;
    _p = p;
    // p = (vec4(p, 1.) * rotationMatrix(vec3(0., 0., 1.), pz*.1)).xyz;
    
    // p = (vec4(p, 1.) * rotationMatrix(vec3(0., 1., 0.), _p.x*.1)).xyz;
    // p = (vec4(p, 1.) * rotationMatrix(vec3(1., 0., 0.), _p.y*.1)).xyz;
    
    p = mod(p, 2.0) - 1.;
    
    float divisor = 1./length(_p.xy)*length(_p.xy);
    
    world = sdSphere(p * divisor, .3);
    world = min(world, length(p.yz) * divisor - .1);
    world = min(world, length(p.xz) * divisor - .1);
    world = min(world, length(p.xy) * divisor - .1);
  ...
  vec3 lighting( vec3 sp, vec3 camPos, int reflectionPass, float dist, float field, vec3 rd) {
    ...
    void main() {
      ...
      vec3 camera_position = vec3(clamp(0.5 - u_mouse.x, -.5, .5) + 2., -6.15 + u_mouse.y - .5, -1.0); // This is the point you look from, or camera you look at the scene through. Whichever way you wish to look at it.
...
function init() {
  container = document.getElementById( 'bg-38' );
