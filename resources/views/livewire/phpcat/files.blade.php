...
{{--        <span class="bg-blue-300 p-2">vk.files.php-cat.com</span>--}}
        Поиск файлов во Вконтакте <sup><A href="https://vk.com" class="underline" target="_blank" >VK.com</a></sup></h2>

    <p class="mb-2">Файлы всего русского комьюнити, можно найти мне кажется всё что угодно,
        можно качать по прямым ссылкам (<u>бесплатное скачивание</u> при подключении доступа к социально значимым сервисам,
        доступно у всех мобильных операторов)</p>

    <p class="text-xl bg-yellow-100 py-5 text-center"><b>НЕ забывайте о безопасности,</b><br/>в файлах могут быть вирусы и всякие обманы, антивирус и бдительность должны быть включены 100%</p>
...
{{--               wire:model.blur="searchTxt"--}}
                />
        {{--        <button class="px-3 py-1 bg-blue-200">Искать</button>--}}
{{--    </form>--}}
<br/>
    Фильтр по размеру файла:
    <select wire:model.live="filterBig" >
...
{{--    $searchTxt: {{ $searchTxt }}--}}
    ...
    @if ( !empty($searchTxt) )
{{--        <div wire:transition>--}}
            <p class="p-2 bg-green-300 text-center">
...
{{--@if ( $resultsSearch )--}}
    ...
    {{--        {{ $count = 0 }}--}}
            @foreach( $resultsSearch as $i )
                ...
                >
                    <td>{{ $count++ }}</td>
                    <td class="p-1"><a href="{{ $i->url }}" class="text-blue-500 hover:underline" target="_blank">{{ $i->title }}
                            @if( $i->ext == 'jpg' || $i->ext == 'gif' || $i->ext == 'png' )
    {{--                        @if( $i->ext == 'jpg' )--}}
                                <img src="{{ $i->url }}" style="max-height: 150px; float:left; padding-right: 5px;"/>
                            @endif
                        </a>
                        ...
                        @if( !empty($i->preview->photo->sizes ) && $i->ext == 'jpg')
                            ...
                            @foreach( $i->preview->photo->sizes as $im )
                                <A href="{{$im->src}}" class="bg-orange-500 m-2 p-2" target="_blank" title="">{{$im->type}}
                                    -> {{$im->width}}x{{$im->height}}</A>
                            @endforeach
                        @endif

                    </td>
                    <td>{{ round($i->size/1024/1024,2) }} Mb</td>
                </tr>
    {{--            {{ $count++ }}--}}
    {{--            <tr>--}}
    {{--                <td colspan="2">--}}
