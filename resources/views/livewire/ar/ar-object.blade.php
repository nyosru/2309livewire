<div
    ...
    <div
        ...
        <span wire:click="switchShowInfo">
        <livewire:helper.showerDopInfo
            :string="$object->nomer .'<sup>'.$object->adres.'</sup>'"
    ...
    @if( $show_info )
        ...
        @foreach($object->prices as $price)
            <div
                class="w-full md:w-[48%] float-left mb-3 mr-[1%] tenant-card mt-4 border border-gray-300 rounded-md p-4">
                <div class="tenant-header flex justify-between">
                    ...
                    <div class="tab-buttons">
                        <button @click="tab = 'payments'" :class="{ 'bg-gray-200': tab === 'payments' }"
                                class="px-4 py-2">
                            Платежи
                        </button>
                        <button @click="tab = 'comments'" :class="{ 'bg-gray-200': tab === 'comments' }"
                                class="px-4 py-2">
                            Комментарии
                        </button>
                    </div>
