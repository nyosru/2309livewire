        ...
        @foreach ($posters as $poster)
            <li class="p-4 bg-gray-100 rounded shadow-md block">
                ...
                <span class="mb-2 text-xl bg-orange-100 p-1"
                    ...
                    @if( !empty($poster->relativeDate) )
                        <span class="bg-orange-400 p-1">{{ $poster->relativeDate  }}</span>
                    @endif
                    ...
                    @if (date('H:i', strtotime($poster->event_time)) !== '00:00')
                        в {{ date('H:i', strtotime($poster->event_time)) }}
                    @endif
</span>
                <br/>
                <br/>

                <a href="{{ route('afisha.detail', ['posterId' => $poster->id]) }}" class="hover:underline">
                ...
                <div class="ml-4">
                    ...
                    <p><a href="{{ $poster->link }}" target="_blank"
                          class="text-blue-500 hover:underline">{{ $poster->link }}</a></p>

                    @if($poster->address)
                        <p>Адрес проведения: <a href="https://2gis.ru/tyumen/search/{{ $poster->address }}"
                                                title="Посмотреть в 2гис" target="_blank"
                                                class="text-blue-500 hover:underline">{{ $poster->address }}</a>
                        </p>
                    @endif
                    ...
                    @if($poster->buy_tickets_link && now()->lt(new \DateTime($poster->event_date . ' ' . $poster->event_time)))
                        <p><a href="{{ $poster->buy_tickets_link }}" class="text-blue-500 hover:underline">Купить
                                билеты</a>
                        </p>
                    @endif
                </div>
